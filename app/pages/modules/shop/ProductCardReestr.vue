      ...
      <Toolbar>
        <template #start>
          ...
          <Button label="Создать" icon="pi pi-plus" class="p-button-success"
            @click="clickNew({ module: 'shop', form: 'ProductCardDoc' })" />
          <p>|</p>
...
const DataStore = useDocHandlerStore('ProductCardDoc');
...
const _clickDelete = async () => {
  const { Error = null, Message = null } = await clickDelete({ module: 'shop', form: 'ProductCardDoc', selectedRows: unref(selectedRows) });
  if (Error) {
